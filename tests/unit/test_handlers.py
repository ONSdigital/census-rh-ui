...
class TestHandlers(RHTestCase):
    ...
    async def test_get_webchat_not_open_clientconnectionerror_en(self):
        ...
        with mock.patch('app.handlers.WebChat.get_now') as mocked_get_now:
            ...
            with aioresponses(passthrough=[str(self.server._root)]) as mocked:
                mocked.get(self.webchatsvc_url,
                           exception=ClientConnectionError('Failed'))

                with self.assertLogs('respondent-home', 'ERROR') as cm:
                    response = await self.client.request(
                        'GET', self.get_webchat_en)
    ...
    async def test_post_request_access_code_enter_address_good_postcode_hh_en(
            self):
        with mock.patch('app.handlers.RequestCodeCommon.get_ai_postcode'
