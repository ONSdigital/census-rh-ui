...
class TestStartHandlers(TestHelpers):
    ...
    @unittest_run_loop
    async def test_start_timeout_en(self):
        with self.assertLogs('respondent-home', 'INFO') as cm:
            response = await self.client.request('GET', self.get_start_timeout_en)
            self.assertLogEvent(cm, "received GET on endpoint 'en/start/timeout'")
            self.assertEqual(response.status, 200)
            contents = str(await response.content.read())
            self.assertIn(self.ons_logo_en, contents)
            self.assertIn('<a href="/cy/start/timeout/" lang="cy" >Cymraeg</a>', contents)
    ...
    @unittest_run_loop
    async def test_start_timeout_cy(self):
        with self.assertLogs('respondent-home', 'INFO') as cm:
            response = await self.client.request('GET', self.get_start_timeout_cy)
            self.assertLogEvent(cm, "received GET on endpoint 'cy/start/timeout'")
            self.assertEqual(response.status, 200)
            contents = str(await response.content.read())
            self.assertIn(self.ons_logo_cy, contents)
            self.assertIn('<a href="/en/start/timeout/" lang="en" >English</a>', contents)
    ...
    @unittest_run_loop
    async def test_start_timeout_ni(self):
        with self.assertLogs('respondent-home', 'INFO') as cm:
            response = await self.client.request('GET', self.get_start_timeout_ni)
            self.assertLogEvent(cm, "received GET on endpoint 'ni/start/timeout'")
            self.assertEqual(response.status, 200)
            contents = str(await response.content.read())
            self.assertIn(self.nisra_logo, contents)
            self.assertIn(self.content_start_timeout_title_en, contents)
